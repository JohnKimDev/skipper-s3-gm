...
module.exports = function SkipperS3 (globalOpts) {
      ...
      client.get(prefix).on('response', function(s3res){
        ...
        // or an error:
          ...
          s3res.once('end', function () {
            ...
            err.message = s3res.statusCode + ' status code returned from S3 for requested file.';
            if (body) err.message += ('\n'+body);
            __transform__.emit('error', err);
          });
        }
  ...
  function S3Receiver (options) {
    ...
    receiver__._write = function onFile(__newFile, encoding, next) {
      ...
      var _gm  = gm(__newFile);

      // ********* GraphicMagick Options **********
      if (options.gm) {
        // EXIF profile
        if (options.gm.noProfile === true) {     // DEFAULT to false
          _gm = _gm.noProfile();    // remove EXIF profile data
        }
        // CROP
        if (options.gm.crop) {
          _gm = _gm.crop(options.gm.crop.width, options.gm.crop.height, options.gm.crop.x, options.gm.crop.y);
        }
        // RESIZE
        if (options.gm.resize) {
          _gm = _gm.resize(options.gm.resize.width, options.gm.resize.height, options.gm.resize.options);
        }
      }

      var fileFormat = options.gm.format || null;
      _gm.stream(fileFormat, function (err, stdout, stderr) {
        if (err) return next(err);

        options.tmpdir = options.tmpdir || path.resolve(process.cwd(), '.tmp/s3-upload');

        var headers = options.headers || {};

        if (options.gm.format) {
          headers['content-type'] = mime.getType(options.gm.format);
        } else if (typeof headers['content-type'] === 'undefined') {
          headers['content-type'] = mime.lookup(__newFile.fd);
        }

        var newFileName = __newFile.fd;
        if (typeof options.filename === 'string') {
          newFileName = options.filename;
        } else if (typeof options.filename === 'function') {
          newFileName = options.filename(__newFile);
        } 

        // console.log('File Object Name for S3:', newFileName);
